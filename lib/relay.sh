...
_relay_main_menu() {
    while true; do
        ...
        echo -e "${CYAN}=== 中转管理 ===${NC}"
        echo ""
        echo "  说明: 中转 = 落地机(你的VPS) → 线路机(中转VPS) → 用户"
        echo "  落地机: 搭建了代理节点的机器（本机通常是落地机）"
        echo "  线路机: 负责转发流量的中转机器"
        echo ""
        echo "  落地机操作:"
        echo "    ${GREEN}[1]${NC} 生成线路机安装脚本（VLESS Reality 中转）"
        echo "    ${GREEN}[2]${NC} 生成全协议中转 Token（发给中转机）"
        echo ""
        echo "  中转机操作:"
        echo "    ${GREEN}[3]${NC} 导入落地 Token（配置中转入站）"
        echo "    ${GREEN}[4]${NC} 查看中转路由"
        echo "    ${GREEN}[5]${NC} 删除中转路由"
        echo ""
        echo "  端口转发 (iptables):"
        echo "    ${GREEN}[6]${NC} 端口转发管理"
        echo ""
